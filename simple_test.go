...
import "github.com/Ari1009/simple"
...
`
	...
	cleaners := make(map[string]simple.ValueCleaner)
	cleaners["constantTransfo"] = &constantValueCleaner{}
	simple := simple.NewsimpleFromConfig(strings.NewReader(config), cleaners)
...
`
	...
	// test
	if _, err := simple.Clean(doc); err == nil {
		// check
		assert.NotNil(t, doc)
...
func TestBson(t *testing.T) {
  ...
  ["numid"]
	...
	cleaners := make(map[string]simple.ValueCleaner)
	cleaners["constantTransfo"] = &constantValueCleaner{}
	cleaner := simple.Newsimple(strings.NewReader(config))
