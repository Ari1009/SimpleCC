...
// simple contains config cleaners
type simple struct {
	root     *configNode
...
// Clean given object
func (simple simple) Clean(value interface{}) (interface{}, error) {
	if simple.root == nil {
		return nil, errors.New("cleaner without root node. Check the simple creation")
	}
	return simple.root.clean(value)
}
...
// Newsimple creates a cleaner with default cleaner methods
func Newsimple(configuration io.Reader) (simple *simple) {
	cleaners := make(map[string]ValueCleaner)
	cleaners["set"] = &Set{}
	cleaners["nil"] = &Nil{}
	cleaners["date"] = &Date{}
	return NewsimpleFromConfig(configuration, cleaners)
}

// NewsimpleFromConfig creates a cleaner with default cleaners
func NewsimpleFromConfig(configuration io.Reader, cleaners map[string]ValueCleaner) (simple *simple) {
	simple = &simple{root: &configNode{pathItem: "root"}, cleaners: cleaners}
	...
	for path, methodInfo := range paths {
		...
		simple.root.addLeaf(path, methodInfo)
	}
	return simple
}
